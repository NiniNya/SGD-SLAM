...
public:
    ...
    bool IsNotDynamic(const int& i);
    bool IsInStatic(const int& i);
    bool IsNotMoving(const int& i);
    bool IsInMoving(const int& i);
    float do_InDynamic(const int& i, const cv::Mat &imDepth,vector<float> TMnum,vector<std::pair<int, float>> KPinfo);
    float do_InMoving(const int& i, const cv::Mat &imDepth,vector<float> TMnum,vector<std::pair<int, float>> KPinfo);
    float do_NotDynamic(const int& i);
    float do_NotMoving(const int& i);
    float do_InStatic(const int& i);
    float get_pointDis(cv::KeyPoint point, cv::Point poi);
    float get_rectDis(cv::Rect2i area, cv::KeyPoint point);
    int get_TMnum(cv::Rect2i area);
    std::pair<int, float> get_KPnum(cv::Rect2i area, const cv::Mat &imDepth);
    float do_InTM(const int& i);
    vector<bool> vbInDynamic_mvKeys;
    vector<float> movingPro;
    vector<float> TMnum;
    vector<std::pair<int, float>> KPinfo;
    vector<float> TMnum_;
    vector<std::pair<int, float>> KPinfo_;
    bool DynamicFlag;
    bool MovingFlag;
    bool StaticFlag;

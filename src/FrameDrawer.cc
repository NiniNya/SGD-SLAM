...
auto FrameDrawer::formatDobleValue(double val, int fixed) {
...
cv::Mat FrameDrawer::DrawFrame(float imageScale)
{
    ...
    vector<cv::Point2f> vOut;
    vector<float> vmovingPro; 
    ...
    else if(state==Tracking::OK) //TRACKING
    {
        ...
        for(int i=0;i<n;i++)
        {
            ...
            {  
                ...
                if(vbMap[i])
                {
                    ...
                    // if  (vmovingPro[i] > 0.9) 
                    // {
                    //     cv::rectangle(im,pt1,pt2,cv::Scalar(0, 128, 255));
                    //     cv::circle(im,point,2,cv::Scalar(0, 128, 255),-1);
                    // }
                    //cv::putText(im, formatDobleValue(vmovingPro[i], 2), point, cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(255, 255, 255), 1);
                    mnTracked++;
                }
        ...
        {   
            if(vbVO[i] || vbMap[i])
            {
                ...
                if(imageScale != 1.f)
                {
                    point = vOut[i]/ imageScale;
                    float px = vOut[i].x / imageScale;
                    float py = vOut[i].y / imageScale;
                    pt1.x=px-r;
                    pt1.y=py-r;
                    pt2.x=px+r;
                    pt2.y=py+r;
...
void FrameDrawer::Update(Tracking *pTracker)
{
    ...
    movingPro=pTracker->mCurrentFrame.movingPro;
    mvOut = pTracker->mCurrentFrame.T_M;

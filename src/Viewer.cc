...
void Viewer::Run()
{
    ...
    while(1)
    {
        ...
        // Yolo识别对象绘制
        {
            ...
            for (auto vit = mmDetectMap.begin(); vit != mmDetectMap.end(); vit++) //遍历所有识别出的对象，first为名字，second为矩形框
            {
            ...
            for (int i = 0; i < mvDynamicArea.size(); i ++) //遍历所有识别出的对象，first为名字，second为矩形框
            {
                        cv::rectangle(toShow, mvDynamicArea[i], cv::Scalar(0, 0, 255),1);
            }

            for (int i = 0; i < mvStaticArea.size(); i ++) //遍历所有识别出的对象，first为名字，second为矩形框
            {
                        cv::rectangle(toShow, mvStaticArea[i], cv::Scalar(255, 0, 0),1);
            }

            for (int i = 0; i < mvMovableArea.size(); i ++) //遍历所有识别出的对象，first为名字，second为矩形框
            {
                        cv::rectangle(toShow, mvMovableArea[i], cv::Scalar(0, 255, 255),1);
            }
        }
